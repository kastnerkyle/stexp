...
class Conv2(nn.Module):
    ...
    def forward(self, x, global_cond=None):
...
class Overtone(nn.Module):
    def __init__(self, wrnn_dims, fc_dims, cond_channels, global_cond_channels, DEVICE="cuda"):
        super().__init__()
        conv_channels = 128
        rnn_channels = 512
    ...
    def forward(self, x, cond, global_cond=None):
        n = x.size(0)
        x_coarse = x[:, :, :1]
    ...
    def generate(self, cond, global_cond=None, n=None, seq_len=None, verbose=False, use_half=False):
        start = time.time()
        if n is None:
            n = cond.size(0)
        if seq_len is None:
        ...
        if use_half:
            std_tensor = torch.tensor([]).to(self.DEVICE).half()
        else:
            std_tensor = torch.tensor([]).to(self.DEVICE)
