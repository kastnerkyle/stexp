...
class WaveRNN(nn.Module) :
    def __init__(self, rnn_dims, fc_dims, feat_dims, aux_dims, DEVICE="cuda"):
        super().__init__()
        self.n_classes = 256
    ...
    def generate(self, feat, aux1=None, aux2=None, aux3=None, deterministic=False, use_half=False, verbose=False, seq_len=None, batch_size=None):
        ...
        h = torch.zeros(batch_size, self.rnn_dims).to(self.DEVICE)
        if use_half:
            h = h.half()

        c_val = torch.zeros(batch_size).to(self.DEVICE)
        f_val = torch.zeros(batch_size).to(self.DEVICE)
        zero = torch.zeros(batch_size).to(self.DEVICE)
