...
class Model(nn.Module) :
    def __init__(self, rnn_dims, fc_dims, global_decoder_cond_dims, upsample_factors, normalize_vq=False,
            noise_x=False, noise_y=False, DEVICE="cuda"):
        super().__init__()
        self.DEVICE = DEVICE
        self.n_classes = 256
        self.overtone = Overtone(rnn_dims, fc_dims, 128, global_decoder_cond_dims, DEVICE=DEVICE)
        self.vq = VectorQuant(1, 512, 128, normalize=normalize_vq, DEVICE=DEVICE)
        ...
        self.encoder = DownsamplingEncoder(128, encoder_layers, DEVICE=DEVICE)
    ...
    def do_train(self, paths, dataset, optimiser, epochs, batch_size, step, lr=1e-4, valid_index=[], use_half=False, do_clip=False):
        ...
        for e in range(epochs) :
            ...
            for i, (speaker, wave16) in enumerate(trn_loader) :
                ...
                if self.noise_y:
                    noisy_f = total_f * (0.02 * torch.randn(total_f.size(0), 1).to(self.DEVICE)).exp() + 0.003 * torch.randn_like(total_f)
                else:
                    noisy_f = total_f
    ...
    def do_generate(self, paths, step, data_path, test_index, deterministic=False, use_half=False, verbose=False):
        ...
        out = self.forward_generate(torch.stack(speakers + list(reversed(speakers)), dim=0).to(self.DEVICE), torch.stack(aligned + aligned, dim=0).to(self.DEVICE), verbose=verbose, use_half=use_half)
