...
class Model(nn.Module) :
    def __init__(self, rnn_dims, fc_dims, pad, upsample_factors, feat_dims, DEVICE="cuda"):
        super().__init__()
        self.n_classes = 256
        self.upsample = UpsampleNetwork(feat_dims, upsample_factors, DEVICE=DEVICE)
    ...
    def do_train(self, paths, dataset, optimiser, epochs, batch_size, step, lr=1e-4, valid_index=[], use_half=False):
        ...
        for e in range(epochs) :
            ...
            for i, (mels, coarse, fine, coarse_f, fine_f) in enumerate(trn_loader) :
                ...
                mels, coarse, fine, coarse_f, fine_f = mels.to(self.DEVICE), coarse.to(self.DEVICE), fine.to(self.DEVICE), coarse_f.to(self.DEVICE), fine_f.to(self.DEVICE)
    ...
    def do_generate(self, paths, step, data_path, test_index, deterministic=False, use_half=False, verbose=False):
        ...
        out = self.forward_generate(torch.stack(aligned).to(self.DEVICE), deterministic, use_half=use_half, verbose=verbose)
