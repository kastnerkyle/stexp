"""Finetuning script for seq2seq text-to-speech synthesis model.
Copied largely from train.py
...
def load_checkpoint(path, model, optimizer, reset_optimizer):
    ...
    try:
        ...
        RuntimeError: Error(s) in loading state_dict for MultiSpeakerTTSModel:
                Missing key(s) in state_dict: "seq2seq.decoder.attention.query_projection.bias", "seq2seq.decoder.attention.query_projection.weight_g", "seq2seq.decoder.attention.query_projection.weight_v". 
                        Unexpected key(s) in state_dict: "seq2seq.decoder.attention.in_projection.bias", "seq2seq.decoder.attention.in_projection.weight_g", "seq2seq.decoder.attention.in_projection.weight_v". 
        """
        b = checkpoint["state_dict"]["seq2seq.decoder.attention.in_projection.bias"].clone()
        g = checkpoint["state_dict"]["seq2seq.decoder.attention.in_projection.weight_g"].clone()
        v = checkpoint["state_dict"]["seq2seq.decoder.attention.in_projection.weight_v"].clone()
        checkpoint["state_dict"]["seq2seq.decoder.attention.query_projection.bias"] = b
        checkpoint["state_dict"]["seq2seq.decoder.attention.query_projection.weight_g"] = g
        checkpoint["state_dict"]["seq2seq.decoder.attention.query_projection.weight_v"] = v
        del checkpoint["state_dict"]["seq2seq.decoder.attention.in_projection.bias"]
        del checkpoint["state_dict"]["seq2seq.decoder.attention.in_projection.weight_g"]
        del checkpoint["state_dict"]["seq2seq.decoder.attention.in_projection.weight_v"]
        print("correcting...")
        model.load_state_dict(checkpoint["state_dict"])
...
if __name__ == "__main__":
    ...
    optimizer = optim.Adam([{"params" : model.seq2seq.parameters(), "lr" : hparams.initial_learning_rate / 1.},
    ...
    """
    optimizer = optim.Adam([{"params" : model.postnet.parameters()},],
                            lr=hparams.initial_learning_rate, betas=(
        hparams.adam_beta1, hparams.adam_beta2),
        eps=hparams.adam_eps, weight_decay=hparams.weight_decay,
        amsgrad=hparams.amsgrad)
